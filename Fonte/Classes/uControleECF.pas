...
uses ZConnection, ZDataset, SysUtils, uConstantes, Classes, uProcedimentosBanco, uExceptions, Variants, Forms,
     Dialogs, Windows, uNumeros, Controls, uNaturezaOperacao, uCodigoFiscal, uCliente;
...
function Bematech_FI_ProgramaIdAplicativoMFD(Aplicativo : String): Integer; StdCall; External 'BEMAFI32.DLL';
...
function  ImprimeCupomFiscal(piCodigoPedido: integer; pzcoConnection: TZConnection):Boolean;
...
var
  ZQryPedido: TZQuery;
  Codigo, Descricao, Aliquota, TipoQtde,
  Qtde, Valor, TipoDesc, Desc,
  AcreDesc,TipoAcreDesc,ValorAcreDesc, sTotal,Mesg: String;
  nTotal: Currency;
...
begin
  ...
  with ZQryPedido do begin
    Connection := pzcoConnection;
    SQL.Clear;
    SQL.Add('select * from tbpedidoitens');
    SQL.Add('join tbproduto on tbproduto.icodigo = tbpedidoitens.iproduto');
    SQL.Add('where tbpedidoitens.ipedido = :ipedido and tbproduto.sservico = ''N''');
    ParamByName('ipedido').AsInteger := piCodigoPedido;
    Open;
    nTotal := 0.00;
    Bematech_FI_AbreCupom( pchar( '' ) );
    while not Eof do begin
      Codigo    := FieldByName('iProduto').AsString;
      Descricao := FieldByName('sdescricao').AsString;
      Aliquota  := '01';
      TipoQtde  := 'F';
      Qtde      := FormatFloat('0.000',FieldByName('nQuantidade').AsFloat);
      Valor     := FormatFloat('0.00',FieldByName('nvalor').AsFloat);
      TipoDesc  := '%';
      Desc      := '00,00';
      nTotal := nTotal + FieldByName('nQuantidade').AsFloat *FieldByName('nvalor').AsFloat;
      screen.Cursor := crHourglass;
      if Length(Descricao) >= 28 then begin
        //Bematech_FI_AumentaDescricaoItem( Descricao );
        Descricao := Copy(Descricao,1,28);
      end;
      Bematech_FI_VendeItem( pchar( Codigo ), pchar( Descricao ), pchar( Aliquota ),
                                       pchar( TipoQtde ), pchar( Qtde ), 2, pchar( Valor ), pchar( TipoDesc ),
                                       pchar( Desc ) );
      Next;
    end;
    SQL.Clear;
    SQL.Add('select * from tbpedido where icodigo = :ipedido');
    ParambyName('ipedido').AsInteger := piCodigoPedido;
    Open;
    AcreDesc     := 'D';
    TipoAcreDesc := '%';
    ValorAcreDesc:= FormatFloat('#,##0.00',FieldByName('najustepercentual').AsFloat);
    nTotal := nTotal - (FieldByName('najustepercentual').AsFloat*nTotal);
    sTotal := FormatFloat('#,##0.00',nTotal);
    Mesg := 'Obrigado, volte sempre !!!';
    screen.Cursor := crHourglass;
    Bematech_FI_FechaCupom( pchar('Dinheiro'), pchar( AcreDesc ),
                            pchar( TipoAcreDesc ), pchar(ValorAcreDesc),
                            pchar(sTotal), Mesg );
    screen.Cursor := crDefault;
    //Grava número do cupom
    for iConta := 1 to 6 do NumeroCupom := NumeroCupom + ' ';
    iRetorno := Bematech_FI_NumeroCupom( NumeroCupom );
    SQL.Clear;
    SQL.Add('update tbpedido set scupomfiscal = :numerocupom where icodigo = :ipedido');
    ParambyName('ipedido').AsInteger    := piCodigoPedido;
    ParambyName('numerocupom').AsString := NumeroCupom;
    ExecSQL;
  end;
end;
